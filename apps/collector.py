...
ALLOWED_REGIONS = get_opted_in_regions()
...
def get_rds_instances():
    return fetch(f"""
        select db_instance_identifier, engine, allocated_storage,
               status, endpoint_address, class, region
...
def get_redshift_clusters():
    return fetch(f"""
        select cluster_identifier, node_type, number_of_nodes, cluster_status,
               db_name, endpoint ->> 'address' as endpoint, region
...
def get_glacier_vaults():
    return fetch(f"""
        select vault_name, creation_date, vault_arn,
               number_of_archives, size_in_bytes, region
...
def get_glue_catalog_database():
    return fetch(f"""
        select name, description, location_uri, create_time, catalog_id, region
...
def get_kinesis_stream():
    return fetch(f"""
        select stream_name, stream_arn, stream_status, open_shard_count, region
...
def get_msk_cluster():
    return fetch(f"""
        select cluster_name, arn, state, current_version as kafka_version, region
